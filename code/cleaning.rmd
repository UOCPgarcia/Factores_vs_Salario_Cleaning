...
```{r}
# edad-income
ggplot(datosOrigen) + aes(x=as.numeric(age), group=salary, fill=salary) + 
  geom_histogram(binwidth=1, color='black')
```

```{r}
table(datosOrigen$sex,datosOrigen$salary)
```

```{r}
qplot(salary, data = datosOrigen, fill = education)+ facet_grid (.~sex)
```
...
qplot(salary, data = datosOrigen, fill = workclass)+ facet_grid (. ~ sex)
```

```{r}
# race-salary
qplot(salary, data = datosOrigen, fill = race)
