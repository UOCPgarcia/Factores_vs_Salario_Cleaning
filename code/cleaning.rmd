...
Se incluyen como ejemplo las primeras filas del dataset y las últimas para ilustrar los datos con los que contamos.   
...
Revisión de las variables para su análisis y limpieza
...
Se localizan valores '?' en el dataset que pasamos a considerar valores na (Not Available), para después ser eliminados.

```{r}

datos$workclass [which(datos$workclass == "?")] <- NA


```

No se encuentran en este dataset, pero sí se localizan caracteres
